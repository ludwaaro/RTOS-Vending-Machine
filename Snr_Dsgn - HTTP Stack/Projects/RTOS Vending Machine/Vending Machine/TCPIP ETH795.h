...
/* New HTTP server with POST, Cookies, Authentication, etc. */
...
/* Dynamic Host Configuration Protocol client for obtaining IP address and
 * other parameters*/
...
/* Simple Network Management Protocol v2C Community Agent */
...
/* TTP Client example in GenericTCPClient.c */
...
/* Domain Name Service Client for resolving hostname strings to IP addresses */
#define STACK_USE_DNS

/* Domain Name Service Server for redirection to the local device */
//#define STACK_USE_DNS_SERVER

/*NetBIOS Name Service Server for repsonding to NBNS hostname broadcast
 * queries*/
//#define STACK_USE_NBNS	

/* Module for resetting this PIC remotely.  Primarily useful for a Bootloader.*/
//#define STACK_USE_REBOOT_SERVER

/* Simple Network Time Protocol for obtaining current date/time from Internet */
#define STACK_USE_SNTP_CLIENT

/* Module for testing UDP TX performance characteristics.  */
 ...
 *      production networks, especially with VPNs (could tunnel broadcast
 *      traffic across a limited bandwidth connection).
 */
/* Module for testing TCP TX performance characteristics */
 ...
 */
#define MY_DEFAULT_HOST_NAME			"chipKIT Pro MX7"
...
#define MY_DEFAULT_MAC_BYTE4  (0x17)	// controller and wish to use the 
#define MY_DEFAULT_MAC_BYTE5  (0xCB)	// internal factory programmed MAC
#define MY_DEFAULT_MAC_BYTE6  (0xF8)	// address instead.
...
// ============ Office ===================
    ...
    #define MY_DEFAULT_GATE_BYTE3    (220ul)
    #define MY_DEFAULT_GATE_BYTE4    (1ul)

    #define MY_DEFAULT_PRIMARY_DNS_BYTE1 (129ul)
    #define MY_DEFAULT_PRIMARY_DNS_BYTE2 (101ul)
    #define MY_DEFAULT_PRIMARY_DNS_BYTE3 (222ul)
    #define MY_DEFAULT_PRIMARY_DNS_BYTE4 (131ul)
    ...
    #define MY_DEFAULT_SECONDARY_DNS_BYTE4 (175ul)
// ===========================================
#endif
...
#define EMAC_RX_DESCRIPTORS 8	/* number of the RX descriptors and RX buffers
                                   to be created */

/* The size of a RX buffer. should be multiple of 16 this is the size of all
 * receive buffers processed by the ETHC. The size should be large enough to
 ...
 * where their TCB, TX FIFO, and RX FIFO should be stored, and how big the RX
 * and TX FIFOs should be.  Making this initializer bigger or smaller defines
 * how many total TCP sockets are available.
 * 
 * Each socket requires up to 56 bytes of PIC RAM and 48+(TX FIFO size) +
 * (RX FIFO size) bytes of TCP_*_RAM each.
 * 
 * Note: The RX FIFO must be at least 1 byte in order to receive SYN and FIN
 * messages required by TCP.  The TX FIFO can be zero if desired. */
...
/* // This slows UDP TX performance by nearly 50%, except when using the
 * ENCX24J600 or PIC32MX6XX/7XX, which have a super fast DMA and incurs
 ...
 * connection).  Undefining this macro results in the Ethernet/Wi-Fi RAM being
 * used (minimum PIC RAM usage, lower performance). Defining this macro results
 * in PIC RAM getting used (higher performance, but uses PIC RAM). This option
 * should not be enabled on PIC18 devices. The performance increase of having
 * this option defined is only apparent when the HTTP server is servicing
...
/* Define which HTTP modules to use. If not using a specific module, comment
...
/* Define the minimum number of bytes free in the TX FIFO before executing
 * callbacks*/
...
/* -- SSL Options -------------------------------------------------------- */
    ...
    #define MAX_SSL_HASHES	(5ul)	/* Max # of SSL hashes  (2 per, plus 1
                                            to avoid deadlock) */

/*  Bits in SSL RSA key.  This parameter is used for SSL sever connections only.
 *   The only valid value is 512 bits (768 and 1024 bits do not work at this
 * time).  Note, however, that SSL client operations do currently work up to
...
/* Default local listening port for the Telnet server.  Port 23 is the protocol
 * default. */

    #define TELNET_PORT			(23u)

/* Default local listening port for the Telnet server when SSL secured. Port
 * 992 is the telnets protocol default. */
