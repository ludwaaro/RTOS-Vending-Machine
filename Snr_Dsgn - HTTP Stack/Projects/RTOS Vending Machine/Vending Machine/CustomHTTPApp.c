 ...
 * Output:          0 if output is complete application-defined otherwise
 * Side Effects:    None
...
{
    TCPPutROMString(sktHTTP,(ROM void*)__DATE__" ""9:00:00");
    return;
}

void HTTPPrint_hostname(void)
{
    TCPPutROMString(sktHTTP,AppConfig.NetBIOSName);
    return;
}
...
void HTTPPrint_machineDesc(void)
{
    ...
    TCPPutROMString(sktHTTP,machineDesc);
...
{
    TCPPutROMString(sktHTTP, Products[item].name);
}
...
void HTTPPrint_status(WORD item)
{
    if(Products[item].stock <10)
        TCPPutROMString(sktHTTP, ( BYTE *) "low");
    else
        TCPPutROMString(sktHTTP, ( BYTE *) "ok");
}
#endif
